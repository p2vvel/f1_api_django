    ...
    def get_constructors(self, instance: Seasons) -> list[str]:
        ...
        try:
            ...
            if instance.year >= 1958:
                ...
                constructors_standings = ConstructorStandings.objects.filter(race__round=last_round, race__year=instance.year).order_by("position").values("constructor__constructorref")
                constructors_urls = [reverse("constructor-detail", args=(c["constructor__constructorref"],)) for c in constructors_standings]
                return constructors_urls
            else:
                # list constructors alphabetically
                teams_query = Results.objects.filter(race__year=1950).order_by("constructor__name").values("constructor__constructorref", "constructor__name").distinct()
                teams_urls = [reverse("constructor-detail", args=(t.get("constructor__constructorref"),)) for t in teams_query]
                return teams_urls
        except Exception as e:
            print(e)
            return []
